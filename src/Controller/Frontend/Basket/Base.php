...
abstract class Base extends \Aimeos\Controller\Frontend\Base implements Iface
{
    ...
    protected function calcPrice(
        \Aimeos\MShop\Order\Item\Product\Iface $orderProduct,
        \Aimeos\Map $prices,
        float $quantity
            ...
            try {
                ...
                foreach ($product->getAttributeItems() as $attrItem) {
                    switch ($attrItem->getType()) {
                        case 'variant': $variantIds[] = $attrItem->getAttributeId();
                            break;
                        case 'config': $configIds[$attrItem->getAttributeId()] = $attrItem->getQuantity();
                            break;
                        case 'custom': $customIds[$attrItem->getAttributeId()] = $attrItem->getValue();
                            break;
                    }
                }
